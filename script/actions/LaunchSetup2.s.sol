...
contract LaunchSetup2 is BaseAction {
    ...
    IVestManager public constant vestManager =
        IVestManager(Protocol.VEST_MANAGER);
    ...
    IFeeDepositController public constant feeDepositController =
        IFeeDepositController(Protocol.FEE_DEPOSIT_CONTROLLER);
    ...
    function setBorrowLimits() public {
        ...
        for (uint256 i = 0; i < pairs.length; i++) {
            ...
            if (
                pairs[i] ==
                address(Protocol.PAIR_FRAXLEND_WBTC_FRXUSD_DEPRECATED)
            ) continue;
            if (
                pairs[i] ==
                address(Protocol.PAIR_CURVELEND_TBTC_CRVUSD_DEPRECATED)
            ) continue;
            if (pairs[i] == address(Protocol.PAIR_CURVELEND_SFRXUSD_CRVUSD))
                limit = 50_000_000e18;
            else if (pairs[i] == address(Protocol.PAIR_FRAXLEND_SCRVUSD_FRXUSD))
                limit = 50_000_000e18;
            else if (pairs[i] == address(Protocol.PAIR_FRAXLEND_SFRXETH_FRXUSD))
                limit = 50_000_000e18;
            else if (pairs[i] == address(Protocol.PAIR_CURVELEND_SDOLA_CRVUSD))
                limit = 10_000_000e18;
            else if (
    ...
    function createLP(uint256 amount) public {
        ...
        console2.log(
            "amount0: %s",
            amount0,
            IERC20(token0).balanceOf(Protocol.TREASURY)
        );
        ...
        // Checks
        require(
            IERC20(token0).allowance(Protocol.TREASURY, address(pool)) > 0,
            "Not enough token0 allowance"
        );
        require(
            rsup.allowance(Protocol.TREASURY, address(pool)) > 0,
            "Not enough rsup allowance"
        );
        require(
            IERC20(token0).balanceOf(Protocol.TREASURY) >= amount0,
            "Not enough token0 balance"
        );
        require(
            rsup.balanceOf(Protocol.TREASURY) >= amount1,
            "Not enough rsup balance"
        );
        ...
        _executeTreasury(
            address(pool),
            abi.encodeWithSelector(
                ICurvePool.add_liquidity.selector,
                amounts,
                0,
                Protocol.TREASURY
            )
    ...
    function updateVestSettingsAndClaim() public returns (uint256) {
        ...
        require(
            rsup.balanceOf(address(Protocol.TREASURY)) > 0,
    ...
    function withdrawFees() public {
        ...
        SimpleRewardStreamer debtemissionStreamer = SimpleRewardStreamer(
            address(Protocol.EMISSIONS_STREAM_PAIR)
        );
        require(
            IERC20(Protocol.GOV_TOKEN).balanceOf(
                address(debtemissionStreamer)
            ) > 0,
            "no emissions on debt emission streamer"
        );
        require(
            debtemissionStreamer.rewardRate() > 0,
            "no reward rate for debt emissions"
        );
    }
