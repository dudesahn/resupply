...
contract DeployResupply is DeployResupplyDao, DeployResupplyProtocol {
    ...
    function deployAll() public isBatch(deployer) {
        if (deployMode == DeployMode.FORK)
            defaultBorrowLimit = 50_000_000 * 1e18; // default for testnets
    ...
    function setupEmissionsReceivers() public {
        ...
        bytes memory result = _executeCore(
            ...
            abi.encodeWithSelector(
                receiverFactory.deployNewReceiver.selector,
                "reUSD Incentives Receiver",
                approvedClaimers
            )
        );
        ...
        console.log(
            "reUSD LP Incentives Receiver deployed at",
            address(reusdLPIncentivesReceiver)
        );
        ...
        _executeCore(
            ...
            abi.encodeWithSelector(
                EmissionsController.registerReceiver.selector,
        ...
        _executeCore(
            address(emissionsController),
            abi.encodeWithSelector(
                EmissionsController.setReceiverWeights.selector,
                receiverIds,
                weights
            )
        );
    ...
    function deployDefaultLendingPairs() public {
        ...
        address pair = deployLendingPair(
            ...
            Constants.Mainnet.CURVELEND_SFRXUSD_CRVUSD_ID
        );
        ...
        pair = deployLendingPair(
            1,
            Constants.Mainnet.FRAXLEND_SFRXETH_FRXUSD,
            address(0),
            0
        );
        ...
        deployLendingPair(
            1,
            address(Constants.Mainnet.FRAXLEND_SUSDE_FRXUSD),
            address(0),
            uint256(0)
        );
        ...
        deployLendingPair(
            1,
            address(Constants.Mainnet.FRAXLEND_SCRVUSD_FRXUSD),
            address(0),
            uint256(0)
        );
    ...
    function configurationStep1() public {
        _executeCore(
            address(pairDeployer),
            abi.encodeWithSelector(
                ResupplyPairDeployer.setCreationCode.selector,
        ...
        _executeCore(
            address(registry),
            abi.encodeWithSelector(
                ResupplyRegistry.setVestManager.selector,
                address(vestManager)
            )
        );
        _executeCore(
            address(registry),
            abi.encodeWithSelector(
                ResupplyRegistry.setTreasury.selector,
                address(treasury)
            )
        );
        _executeCore(
            address(registry),
            abi.encodeWithSelector(
                ResupplyRegistry.setStaker.selector,
                address(staker)
            )
        );
    }
    ...
    function configureProtocolContracts() public {
        ...
        _executeCore(
            address(feeDeposit),
            abi.encodeWithSelector(
                feeDeposit.setOperator.selector,
        ...
        _executeCore(
            address(debtReceiver),
            abi.encodeWithSelector(
                SimpleReceiver.setApprovedClaimer.selector,
                address(rewardHandler),
                true
            )
        );
        ...
        _executeCore(
            address(registry),
            abi.encodeWithSelector(
                ResupplyRegistry.setRedemptionHandler.selector,
        ...
        _executeCore(
            address(registry),
            abi.encodeWithSelector(
                ResupplyRegistry.setLiquidationHandler.selector,
        ...
        _executeCore(
            address(registry),
            abi.encodeWithSelector(
                ResupplyRegistry.setInsurancePool.selector,
        ...
        _executeCore(
            address(registry),
            abi.encodeWithSelector(
                ResupplyRegistry.setFeeDeposit.selector,
                address(feeDeposit)
            )
        );
        _executeCore(
            address(registry),
            abi.encodeWithSelector(
                ResupplyRegistry.setRewardHandler.selector,
                address(rewardHandler)
            )
        );
        _executeCore(
            address(stablecoin),
            abi.encodeWithSelector(
                Stablecoin.setOperator.selector,
                address(registry),
                true
            )
        );
    ...
    function deployCurvePools() public {
        ...
        result = addToBatch(
            ...
            abi.encodeWithSelector(
                ICurveExchange.deploy_plain_pool.selector,
                "reUSD/scrvUSD", // name
                "reusdscrv", // symbol
                coins, // coins
                200, // A
        ...
        result = addToBatch(
            address(Constants.Mainnet.CURVE_STABLE_FACTORY),
            abi.encodeWithSelector(
                ICurveExchange.deploy_gauge.selector,
                crvusdPool
            )
        );
    ...
    function deploySwapper() public {
        ...
        bytes32 salt = buildGuardedSalt(
            ...
            true,
            false,
            uint88(uint256(keccak256(bytes("Swapper"))))
        );
        bytes memory bytecode = abi.encodePacked(
            vm.getCode("Swapper.sol:Swapper"),
            abi.encode(address(core), address(registry))
        );
        ...
        if (addressHasCode(predictedAddress))
            revert("Swapper already deployed");
        addToBatch(
        ...
        _executeCore(
            address(registry),
            abi.encodeWithSelector(
                registry.setDefaultSwappers.selector,
                swappers
            )
        );
