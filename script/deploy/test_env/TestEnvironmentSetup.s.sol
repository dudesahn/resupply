...
contract TestEnvironmentSetup is DeployResupply {
    function run() public override isBatch(deployer) {
        ...
        rateCalculator = InterestRateCalculator(
            0x3b7AbCB8E1d7E2F1ba89BF5Eec037F07F2ed2CCF
        );
        pairDeployer = ResupplyPairDeployer(
            0x5555555524De7C56C1B20128dbEAace47d2C0417
        );
        registry = IResupplyRegistry(
    ...
    function handoffGovernance() public {
        ...
        _executeCore(
            address(core),
            abi.encodeWithSelector(ICore.setVoter.selector, _voter)
        );
    }
    ...
    function issueTokens() public {
        ...
        console.log(
            "stablecoin balance of deployer",
            IERC20(_stablecoin).balanceOf(deployer)
        );
    }
    ...
    function provideLiquidity() public isBatch(deployer) {
        ...
        addToBatch(
            crvusdPool,
            abi.encodeWithSelector(
                ICurveExchange.add_liquidity.selector,
                amounts,
                0,
                deployer
            )
