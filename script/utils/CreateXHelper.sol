...
abstract contract CreateXHelper {
    ICreateXDeployer public constant createXFactory =
        ICreateXDeployer(0xba5Ed099633D3B313e4D5F7bdc1305d3c28ba5Ed);

    function encodeCREATE3Deployment(
        bytes32 salt,
        bytes memory initCode
    ...
    function encodeCREATE2Deployment(
        bytes32 salt,
        bytes memory initCode
    ) public pure returns (bytes memory) {
        return
    ...
    function encodeCREATEDeployment(
        bytes memory initCode
    ) public pure returns (bytes memory) {
        return
            abi.encodeWithSelector(
    ...
    function computeCreate3AddressFromSaltPreimage(
        bytes32 saltPreimage,
        address deployer,
        ...
        if (enablePermissionedDeploy && !enableCrossChainProtection) {
            return
                createXFactory.computeCreate3Address(
                    _efficientHash({
            ...
            return
                createXFactory.computeCreate3Address(
                    keccak256(
                        abi.encode(msg.sender, block.chainid, saltPreimage)
                    )
                );
        } else if (!enablePermissionedDeploy && enableCrossChainProtection) {
            return
            ...
            return
                createXFactory.computeCreate3Address(
                    keccak256(abi.encode(saltPreimage))
                );
        }
    }

    function computeCreate2AddressFromSaltPreimage(
        bytes32 saltPreimage,
        bytes memory initCode
    ) public view returns (address) {
        return
            createXFactory.computeCreate2Address(
                keccak256(abi.encode(saltPreimage)),
                keccak256(initCode)
            );
    }

    function isAlreadyDeployedCreate2(
        bytes32 saltPreimage,
        bytes memory initCode
    ) public view returns (bool) {
        return
            addressHasCode(
                computeCreate2AddressFromSaltPreimage(saltPreimage, initCode)
            );
    }

    function isAlreadyDeployedCreate3(
        bytes32 saltPreimage,
        address deployer,
        bool enablePermissionedDeploy,
        bool enableCrossChainProtection
        ...
        return
            addressHasCode(
                computeCreate3AddressFromSaltPreimage(
                    saltPreimage,
                    deployer,
                    enablePermissionedDeploy,
                ...
                ((
                    ...
                    ((
                        enableCrossChainProtection
                            ? bytes32(uint256(1))
                            : bytes32(0)
                    ) << 88) | // left 160 bits specify permissioned deployer // byte specifing 0x01 for cross-chain protection; 0x00 for none
