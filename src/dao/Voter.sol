...
contract Voter is CoreOwnable, DelegatedOps, EpochTracker {
    ...
    mapping(address account => mapping(uint256 id => Vote vote))
        public accountVoteWeights;
    ...
    */
    ...
    function createNewProposal(
    ...
    ) external callerOrDelegated(account) returns (uint256) {
        ...
        require(
            bytes(description).length <= MAX_DESCRIPTION_BYTES,
            "Description too long"
        );
        if (_containsProposalCancelerPayload(payload))
            require(payload.length == 1, "Payload length not 1");
    ...
    function _voteForProposal(
        ...
        uint256 accountWeight = staker.getAccountWeightAt(
            account,
            proposal.epoch
        ) / 10 ** TOKEN_DECIMALS;
    ...
    function cancelProposal(uint256 id) external onlyOwner {
        ...
        if (proposalPayload[id].length == 1)
            require(
                !_containsProposalCancelerPayload(proposalPayload[id]),
                "Contains canceler payload"
            );
    ...
    function _containsProposalCancelerPayload(
        Action[] memory payload
        ...
        for (uint256 i = 0; i < payloadLength; i++) {
            ...
            if (
                action.target == address(core) &&
                selector == ICore.setOperatorPermissions.selector
            ) {
                ...
                bytes memory slicedData = BytesLib.slice(
                    data,
                    4,
                    data.length - 4
                );
                (, address target, bytes4 permissionSelector, , ) = abi.decode(
                    slicedData,
                    (address, address, bytes4, bool, address)
                );
                if (permissionSelector != this.cancelProposal.selector)
                    continue;
    ...
    function _canExecute(
        ...
        if (
            block.timestamp <
            proposal.createdAt + VOTING_PERIOD + EXECUTION_DELAY
        ) return false;
        if (block.timestamp > proposal.createdAt + EXECUTION_DEADLINE)
            return false;

        // Ensure proposal has met quorum and received more weight in favor.
        if (!_quorumReached(proposal.quorumWeight, proposal.results))
            return false;
        if (proposal.results.weightYes <= proposal.results.weightNo)
            return false;
     ...
     */
    function setMinCreateProposalPct(
        uint256 pct
