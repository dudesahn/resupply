...
contract EmissionsController is CoreOwnable, EpochTracker {
    ...
    event EmissionsAllocated(
        address indexed receiver,
        uint256 indexed epoch,
        bool indexed unallocated,
        uint256 amount
    );
    ...
    constructor(
        ...
        require(
            _emissionsSchedule[0] >= _tailRate,
            "Final rate less than tail rate"
        );
        for (uint256 i = 0; i < _emissionsSchedule.length - 1; i++) {
            require(
                _emissionsSchedule[i] <= _emissionsSchedule[i + 1],
                "Rate greater than predecessor"
            );
        }
     ...
     */
    function setReceiverWeights(
        uint256[] memory _receiverIds,
    ...
    function transferFromAllocation(
        address _recipient,
        uint256 _amount
    ...
    function _fetchEmissions(address _receiver) internal returns (uint256) {
        ...
        while (_allocated.lastAllocEpoch < epoch) {
            ...
            amount = ((receiver.weight *
                emissionsPerEpoch[_allocated.lastAllocEpoch]) / BPS);
            totalMinted += amount;
            emit EmissionsAllocated(
                _receiver,
                _allocated.lastAllocEpoch,
                !receiver.active,
                amount
    ...
    function _calcEmissionsForEpoch(uint256 _epoch) internal returns (uint256) {
        ...
        return ((govToken.globalSupply() * _emissionsRate * epochLength) /
            365 days /
            PRECISION);
    }

    function isRegisteredReceiver(
        address _receiver
