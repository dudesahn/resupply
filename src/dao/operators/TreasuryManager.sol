...
contract TreasuryManager is CoreOwnable {
    ...
    address public constant prismaFeeReceiver =
    ...
    function retrieveETH(address _to) external onlyManager {
        core.execute(
            treasury,
            abi.encodeWithSelector(ITreasury.retrieveETH.selector, _to)
        );
    }

    function retrieveETHExact(
        address _to,
        uint256 _amount
    ) external onlyManager {
        core.execute(
    ...
    ) external onlyManager returns (bool, bytes memory result) {
        result = core.execute(
            treasury,
            abi.encodeWithSelector(ITreasury.execute.selector, _target, _data)
        );
    ...
    function transferTokenFromPrismaFeeReceiver(
        address token,
        address to,
        uint256 amount
    ) external onlyManager {
    ...
    function approveTokenFromPrismaFeeReceiver(
        address token,
        address spender,
        uint256 amount
    ) external onlyManager {
    ...
    function setLpIncentivesReceiver(
        address _lpIncentivesReceiver
    ) external onlyManager {
    ...
    */
    function viewPermissions()
        external
        view
        returns (bool, bool, bool, bool, bool, bool, bool, bool, bool)
    {
