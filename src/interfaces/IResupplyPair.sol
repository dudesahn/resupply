...
interface IResupplyPair {
    ...
    function addCollateralUnderlying(
        uint256 _collateralAmount,
        address _borrower
    ) external;
    ...
    function exchangeRateInfo()
        external
        view
        returns (address oracle, uint32 lastTimestamp, uint224 exchangeRate);
    ...
    function getUserSnapshot(
        ...
        external
        view
        returns (uint256 _userBorrowShares, uint256 _userCollateralBalance);
    ...
    function redeemCollateral(
        ...
        uint256 _fee,
        address _receiver
    ) external returns (address _collateralToken, uint256 _collateralReturned);
    ...
    function rewardMap(
        address _reward
    ) external view returns (uint256 _rewardSlot);
