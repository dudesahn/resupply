...
interface ICurvePoolVirtualPriceOracleWithMinMax is IERC165 {
    event SetMaximumCurvePoolVirtualPrice(
        uint256 oldMaximum,
        uint256 newMaximum
    );
    event SetMinimumCurvePoolVirtualPrice(
        uint256 oldMinimum,
        uint256 newMinimum
    );
    ...
    function CURVE_POOL_VIRTUAL_PRICE_PRECISION()
        external
        view
        returns (uint256);

    function getCurvePoolVirtualPrice()
        external
        view
