...
contract InsurancePool is RewardDistributorMultiEpoch, CoreOwnable {
    ...
    function burnAssets(uint256 _amount) external {
        require(
            msg.sender == IResupplyRegistry(registry).liquidationHandler(),
            "!liq handler"
        );
    ...
    function _clearWithdrawQueueGuarded(
        address _account
    ...
    function _checkWithdrawReady(address _account) internal view {
        ...
        require(
            block.timestamp <= exitTime + withdrawTimeLimit,
            "withdraw time over"
        );
    }
