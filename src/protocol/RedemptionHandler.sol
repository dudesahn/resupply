...
contract RedemptionHandler is CoreOwnable {
    ...
    function setDiscountInfo(
        ...
        require(
            _maxDiscount <= baseRedemptionFee,
            "max discount exceeds base redemption fee"
        );
    ...
    function getMaxRedeemableDebt(
        address _pair
    ) external view returns (uint256) {
        (, , , IResupplyPair.VaultAccount memory _totalBorrow) = IResupplyPair(
            _pair
        ).previewAddInterest();
    ...
    function _getRedemptionFee(
        ...
        returns (
            ...
            RedeemptionRateInfo memory _rdata,
        ...
        )
    {
        ...
        if (_totalBorrow.amount != 0)
            weightOfRedeem = (_amount * PRECISION) / _totalBorrow.amount;
        ...
        if (_rdata.timestamp != 0) {
            ...
            uint192 decay = uint192(
                (block.timestamp - _rdata.timestamp) * usageDecayRate
            );
        ...
        if (_totalweight + _rdata.usage > 0) {
            ...
            uint256 ratio = (_rdata.usage * 10_000) /
                (_totalweight + _rdata.usage);
            ...
            if (ratio > _start) {
                ...
                //clamp to max
                overusageFee = overusageFee > usagediff
                    ? usagediff
                    : overusageFee;
        ...
        if (underlyingOracle != address(0)) {
            uint256 price = IOracle(underlyingOracle).getPrices(
                IResupplyPair(_pair).underlying()
            );
            if (price > 1e18) {
    ...
    function redeemFromPair(
        ...
        (
            uint256 feePct,
            RedeemptionRateInfo memory rdata,
            uint256 _newTotalWeight
        ) = _getRedemptionFee(_pair, _amount);
        ...
        (address _collateral, uint256 _returnedCollateral) = IResupplyPair(
            _pair
        ).redeemCollateral(msg.sender, _amount, feePct, returnToAddress);
    ...
    function previewRedeem(
        ...
        view
        returns (
            uint256 _returnedUnderlying,
            uint256 _returnedCollateral,
            uint256 _fee
        )
    {
        //get fee
        ...
        _returnedUnderlying = IERC4626(collateralVault).previewRedeem(
            _returnedCollateral
        );
    }
}
