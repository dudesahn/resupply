...
abstract contract RewardDistributorMultiEpoch is ReentrancyGuard {
    ...
    mapping(uint256 => mapping(address => uint256))
        public global_reward_integral; //epoch -> token -> integral
    mapping(uint256 => mapping(address => mapping(address => uint256)))
        public reward_integral_for; // epoch -> token -> account -> integral
    ...
    uint256 private constant PRECISION = 1e22;
    ...
    function _calcRewardIntegral(
        ...
        if (
            _epoch == _currentEpoch &&
            _totalRewardShares() > 0 &&
            bal > remainingRewards
        ) {
            uint256 rewardPerToken = (((bal - remainingRewards) * PRECISION) /
                _totalRewardShares());
            if (rewardPerToken > 0) {
                //increase integral
        ...
        if (_account != address(0)) {
            ...
            if (_claimTo != address(0) || userI < reward_global) {
                ...
                if (
                    _epoch == _currentEpoch &&
                    _claimTo != address(0) &&
                    !reward.is_non_claimable
                ) {
                    uint256 receiveable = claimable_reward[rewardToken][
                        _account
                    ] +
                        ((_userRewardShares(_account) *
                            (reward_global - userI)) / PRECISION);
                    if (receiveable > 0) {
                    ...
                    claimable_reward[rewardToken][_account] =
                        claimable_reward[rewardToken][_account] +
                        ((_userRewardShares(_account) *
                            (reward_global - userI)) / PRECISION);
                }
                reward_integral_for[_epoch][rewardToken][
                    _account
                ] = reward_global;
            }
        }
    ...
    function _checkpoint(
        ...
        for (uint256 loops = 0; loops < _maxloops; ) {
            ...
            if (_account != address(0)) {
                ...
                if (
                    userEpoch != globalEpoch && _userRewardShares(_account) == 0
                ) {
    ...
    function earned(
        address _account
    ) public virtual nonReentrant returns (EarnedData[] memory claimable) {
