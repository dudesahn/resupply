...
contract RewardHandler is CoreOwnable, EpochTracker {
    ...
    constructor(
        ...
        insuranceEmissionReceiver = ISimpleReceiver(
            IInsurancePool(insurancepool).emissionsReceiver()
        );
    ...
    function setPairWeight(address _pair, uint256 _amount) external {
        require(
            msg.sender == IResupplyRegistry(registry).feeDeposit(),
            "!feeDeposist"
        );
    ...
    function queueInsuranceRewards() external {
        ...
        require(
            msg.sender == IFeeDeposit(feeDeposit).operator(),
            "!feeDepositOperator"
        );

        //queue up any reward tokens currently on this handler
        IRewards(insuranceRevenue).queueNewRewards(
        ...
        if (insuranceEmissionReceiver.claimableEmissions() > 0) {
            ...
            IRewards(insuranceEmissions).queueNewRewards(
                IERC20(emissionToken).balanceOf(address(this))
            );
        }
    }
    ...
    function queueStakingRewards() external {
        ...
        if (revenueBalance > 0) {
            IGovStaker(govStaker).notifyRewardAmount(
                revenueToken,
                revenueBalance
            );
        }
        ...
        if (debtEmissionsReceiver.claimableEmissions() > 0) {
            ...
            IRewards(pairEmissions).queueNewRewards(
                IERC20(emissionToken).balanceOf(address(this))
            );
        }
    }
