...
contract Swapper is CoreOwnable, ReentrancyGuard {
    ...
    function addPairing(
        ...
        if (previousInfo.swappool != address(0)) {
    ...
    function swap(
        ...
        for (uint256 i = 0; i < path.length - 1; ) {
            ...
            if (swapinfo.swaptype == TYPE_UNDEFINED) {
                ...
                address registeredPair = IResupplyRegistry(registry)
                    .pairsByName(IERC20Metadata(msg.sender).name());
            ...
            if (swapinfo.swaptype == TYPE_DEPOSIT) {
                ...
                IERC4626(swapinfo.swappool).redeem(
                    balanceIn,
                    returnAddress,
                    address(this)
                );
                ...
                ICurveExchange(swapinfo.swappool).exchange(
                    int128(swapinfo.tokenInIndex),
                    int128(swapinfo.tokenOutIndex),
                    balanceIn,
                    0,
                    returnAddress
                );
