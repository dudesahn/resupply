...
contract Utilities is ResupplyPairConstants {
    address public constant convexPoolUtil =
        address(0x5Fba69a794F395184b5760DAf1134028608e5Cd1);
    ...
    function getSwapRouteAmountOut(
        uint256 _amount,
        address _swapper,
        address[] calldata _path
    ) public view returns (uint256 _returnAmount) {
        ...
        for (uint256 i = 0; i < _path.length - 1; ) {
            (
                ...
                int32 tokenOutIndex,
                uint32 swaptype
            ) = ISwapper(_swapper).swapPools(_path[i], _path[i + 1]);
            ...
            if (swaptype == TYPE_DEPOSIT) {
                ...
                _returnAmount = IERC4626(swappool).previewDeposit(
                    _returnAmount
                );
                ...
                _returnAmount = ICurveExchange(swappool).get_dy(
                    int128(tokenInIndex),
    ...
    function isSolvent(
        ...
        uint256 userBorrowShares = IResupplyPair(_pair).userBorrowShares(
            _account
        );
        uint256 borrowerAmount = IResupplyPair(_pair).toBorrowAmount(
            userBorrowShares,
            true,
            true
        );
        ...
        //get collateral
        uint256 collateralAmount = IResupplyPair(_pair).userCollateralBalance(
            _account
        );
        ...
        uint256 _ltv = ((borrowerAmount * exchangeRate * LTV_PRECISION) /
            EXCHANGE_PRECISION) / collateralAmount;
        return _ltv <= _maxLTV;
    }

    //check if a user is solvent after a theoretical leveraged borrow
    //maxltv passed in to simulate enforcing a certain CR
    function isSolventAfterLeverage(
        address _pair,
        uint256 _maxLTV,
        address _account,
        uint256 _addUnderlying,
        ...
        //add underlying
        if (_addUnderlying > 0) {
            collateralAmount += IERC4626(collateralVault).previewDeposit(
                _addUnderlying
            );
        }

        //get exchange rate of the amount borrowed to collateral amount
        uint256 collateralReceived = getSwapRouteAmountOut(
            _borrowAmount,
            _swapper,
            _path
        );
    ...
    function pairCollateralRewardRates(
        address _pair
    ) public view returns (address[] memory tokens, uint256[] memory rates) {
    ...
    //note: has an extra 1e18 decimals of precision
    function getInsurancePoolRewardRates()
        external
        view
        returns (address[] memory tokens, uint256[] memory rates)
    {
        ...
        address insuranceEmissions = IRewardHandler(rewardHandler)
            .insuranceEmissions();
        address insuranceRevenue = IRewardHandler(rewardHandler)
            .insuranceRevenue();
        ...
        uint256 poolWeight = IRewards(insuranceEmissions).balanceOf(
            insurancePool
        );
        if (totalWeight == 0) {
            totalWeight = 1;
