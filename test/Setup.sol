...
contract Setup is Test {
    ...
    address internal constant FRAX_VEST_TARGET =
    ...
    function deployRewardsContracts() public {
        ...
        address simpleReceiverImplementation = address(
            new SimpleReceiver(address(core), address(emissionsController))
        );
        receiverFactory = new SimpleReceiverFactory(
            address(core),
        ...
        debtReceiver = SimpleReceiver(
            receiverFactory.deployNewReceiver("Debt Receiver", new address[](0))
        );
        vm.prank(address(core));
        insuranceEmissionsReceiver = SimpleReceiver(
            receiverFactory.deployNewReceiver(
                "Insurance Receiver",
                new address[](0)
            )
        );
        ...
        insuranceEmissionsReceiver.setApprovedClaimer(
            address(rewardHandler),
            true
        );
        vm.stopPrank();
    ...
    function setInitialEmissionReceivers() public {
        ...
        emissionsController.registerReceiver(
            address(insuranceEmissionsReceiver)
        );
        uint256 debtReceiverId = emissionsController.receiverToId(
            address(debtReceiver)
        );
        uint256 insuranceReceiverId = emissionsController.receiverToId(
            address(insuranceEmissionsReceiver)
        );
    ...
    function deployDaoContracts() public {
        ...
        address vestManagerAddress = vm.computeCreateAddress(
            address(this),
    ...
    function deployDefaultLendingPairs() public {
        //curve lend
        testPair = deployLendingPair(
            0,
            address(Constants.Mainnet.CURVELEND_SDOLA_CRVUSD),
            address(Constants.Mainnet.CONVEX_BOOSTER),
            uint256(Constants.Mainnet.CURVELEND_SDOLA_CRVUSD_ID)
        );
        testPair2 = deployLendingPair(
            0,
            address(Constants.Mainnet.CURVELEND_SUSDE_CRVUSD),
            address(Constants.Mainnet.CONVEX_BOOSTER),
            uint256(Constants.Mainnet.CURVELEND_SUSDE_CRVUSD_ID)
        );
        deployLendingPair(
            0,
            address(Constants.Mainnet.CURVELEND_USDE_CRVUSD),
            address(Constants.Mainnet.CONVEX_BOOSTER),
            uint256(Constants.Mainnet.CURVELEND_USDE_CRVUSD_ID)
        );
        deployLendingPair(
            0,
            address(Constants.Mainnet.CURVELEND_TBTC_CRVUSD_DEPRECATED),
            address(Constants.Mainnet.CONVEX_BOOSTER),
            uint256(Constants.Mainnet.CURVELEND_TBTC_CRVUSD_ID)
        );
        deployLendingPair(
            0,
            address(Constants.Mainnet.CURVELEND_WBTC_CRVUSD),
            address(Constants.Mainnet.CONVEX_BOOSTER),
            uint256(Constants.Mainnet.CURVELEND_WBTC_CRVUSD_ID)
        );
        deployLendingPair(
            0,
            address(Constants.Mainnet.CURVELEND_WETH_CRVUSD),
            address(Constants.Mainnet.CONVEX_BOOSTER),
            uint256(Constants.Mainnet.CURVELEND_WETH_CRVUSD_ID)
        );
        deployLendingPair(
            0,
            address(Constants.Mainnet.CURVELEND_WSTETH_CRVUSD),
            address(Constants.Mainnet.CONVEX_BOOSTER),
            uint256(Constants.Mainnet.CURVELEND_WSTETH_CRVUSD_ID)
        );
        deployLendingPair(
            0,
            address(Constants.Mainnet.CURVELEND_SFRXUSD_CRVUSD),
            address(Constants.Mainnet.CONVEX_BOOSTER),
        ...
        deployLendingPair(
            1,
            address(Constants.Mainnet.FRAXLEND_SFRXETH_FRXUSD),
            address(0),
            uint256(0)
        );
        deployLendingPair(
    ...
    function deployCurvePools() public {
        ...
        address crvusdAmm = ICurveExchange(
        ...
        address fraxAmm = ICurveExchange(Constants.Mainnet.CURVE_STABLE_FACTORY)
            .deploy_plain_pool(
                "reUSD/sfrxUSD", //name
                "reusdsfrx", //symbol
                coins, //coins
