...
contract TreasuryManagerTest is Setup {
    ...
    address public prismaFeeReceiver =
        0xfdCE0267803C6a0D209D3721d2f01Fd618e9CBF8;
    IPrismaCore public prismaCore =
        IPrismaCore(0x5d17eA085F2FF5da3e6979D5d26F1dBaB664ccf8);
    ...
    function test_SetTokenApproval() public {
        ...
        assertEq(
            IERC20(token).allowance(address(treasury), spender),
            TEST_AMOUNT
        );
    }
    ...
    function test_TransferTokenFromPrismaFeeReceiver() public {
        ...
        vm.prank(dev);
        treasuryManager.transferTokenFromPrismaFeeReceiver(
            address(stablecoin),
            address(user1),
            100e18
        );
    ...
    function test_ApproveTokenFromPrismaFeeReceiver() public {
        vm.prank(dev);
        treasuryManager.approveTokenFromPrismaFeeReceiver(
            address(stablecoin),
            address(user1),
            100e18
        );
        assertEq(
            stablecoin.allowance(address(prismaFeeReceiver), address(user1)),
            100e18
        );
    }
    ...
    function test_ViewPermissions() public {
        ...
        assertEq(
            retrieveTokenExact,
            true,
            "retrieveTokenExact permission not set"
        );
        ...
        assertEq(
            ...
            true,
            "transferTokenFromPrismaFeeReceiver permission not set"
        );
        assertEq(
            approveTokenFromPrismaFeeReceiver,
            true,
            "approveTokenFromPrismaFeeReceiver permission not set"
        );
        ...
        assertEq(
            transferTokenFromPrismaFeeReceiver,
            false,
            "transferTokenFromPrismaFeeReceiver still set"
        );
        assertEq(
            approveTokenFromPrismaFeeReceiver,
            false,
            "approveTokenFromPrismaFeeReceiver still set"
        );
    }
