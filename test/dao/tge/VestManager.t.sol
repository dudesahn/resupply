...
contract VestManagerHarness is Setup {
    ...
    function setUp() public override {
        super.setUp();
        autoStakeCallback = new AutoStakeCallback(
            address(core),
            address(staker),
            address(vestManager)
        );
        ...
        VestManagerInitParams.InitParams memory params = VestManagerInitParams
            .getInitParams(
    ...
    function test_VestDurations() public {
        assertEq(
            vestManager.durationByType(VestManager.AllocationType.TREASURY),
            5 * 365 days,
            "TREASURY not 5 years"
        );
        assertEq(
            vestManager.durationByType(VestManager.AllocationType.PERMA_STAKE),
            5 * 365 days,
            "PERMA_STAKE not 5 years"
        );
        assertEq(
            vestManager.durationByType(VestManager.AllocationType.LICENSING),
            1 * 365 days,
            "LICENSING not 1 years"
        );
        assertEq(
            vestManager.durationByType(VestManager.AllocationType.REDEMPTIONS),
            5 * 365 days,
            "REDEMPTIONS not 5 years"
        );
        assertEq(
            vestManager.durationByType(VestManager.AllocationType.AIRDROP_TEAM),
            1 * 365 days,
            "AIRDROP_TEAM not 1 year"
        );
        assertEq(
            vestManager.durationByType(
                VestManager.AllocationType.AIRDROP_VICTIMS
            ),
            2 * 365 days,
            "AIRDROP_VICTIMS not 2 years"
        );
        assertEq(
            vestManager.durationByType(
            ...
            5 * 365 days,
            "AIRDROP_LOCK_PENALTY not 5 years"
        );
        ...
        ) = vestManager.getSingleVestData(address(treasury), 0);
        ...
        (total, claimable, claimed, timeRemaining) = vestManager
            .getSingleVestData(address(permaStaker1), 0);
        assertEq(claimable, 0, "claimable not 0");
        assertGt(total, 0, "total not > 0");
        assertEq(claimed, 0, "claimed not 0");
        assertEq(timeRemaining, 5 * 365 days, "timeRemaining not 5 years");
        ...
        (total, claimable, claimed, timeRemaining) = vestManager
            .getSingleVestData(address(permaStaker2), 0);
        assertEq(claimable, 0, "claimable not 0");
        assertGt(total, 0, "total not > 0");
        assertEq(claimed, 0, "claimed not 0");
        assertEq(timeRemaining, 5 * 365 days, "timeRemaining not 5 years");
        ...
        (total, claimable, claimed, timeRemaining) = vestManager
            .getSingleVestData(FRAX_VEST_TARGET, 0);
        assertEq(claimable, 0, "claimable not 0");
        assertGt(total, 0, "total not > 0");
        assertEq(claimed, 0, "claimed not 0");
        ...
        (total, claimable, claimed, timeRemaining) = vestManager
            .getSingleVestData(address(treasury), 0);
    ...
    function test_SetInitialParams() public {
        ...
        for (
            uint256 i = 0;
            i < uint256(type(VestManager.AllocationType).max);
            i++
        ) {
            VestManager.AllocationType allocationType = VestManager
                .AllocationType(i);
            ...
            if (
                ...
                (
                    ...
                    uint256 _amount,
                    uint256 _claimed
                ) = vestManager.userVests(targets[0], 0);
        ...
        uint expectedRedemptionRatio = ((vestManager.allocationByType(
            VestManager.AllocationType.REDEMPTIONS
        ) * 1e18) / maxRedeemable);
    ...
    function test_AirdropClaim() public {
        ...
        bytes32 sampleRoot = vestManager.merkleRootByType(
            VestManager.AllocationType.AIRDROP_TEAM
        );
    ...
    function getSampleMerkleClaimData()
        public
        pure
        returns (
            address[] memory users,
        ...
        )
    {
        ...
        proofs[0][
            0
        ] = 0x0034dc482e290250c7b0a532700e18918269c66b4d03a6855ed486c00d3cf9ef;
        proofs[0][
            1
        ] = 0xdf9ea50af697b8a23e4a0a81780a3a87eb956d8324f4594e668192977f213238;
        proofs[0][
            2
        ] = 0x504eb35476a7a64c378c228c51ca28ceb4d6b568a382d5ca5c941fbc42700835;
        proofs[0][
            3
        ] = 0x8e14177290ecff25415e54d5ed6ede02ef08f64d9bde9687b9fa7b9642d56e9e;
        proofs[0][
            4
        ] = 0x3ef1a0430cebb90c18513e5af6edbb4e511d7d40e1ced214963cb42870ed0b8c;
        ...
        proofs[1][
            0
        ] = 0x438be342e52ba933e6f3aaeed9b8e29f52f5a47ae491dbfca5a4a525d8663df5;
        proofs[1][
            1
        ] = 0x0970829dec67afd767644142dc22b1ae45237a719591e37755406cbaac2e3165;
        proofs[1][
            2
        ] = 0xe14c7f170b2316ef588bb377424eac8e44e9e1c93f0fd606a4e37926f7f790cc;
        proofs[1][
            3
        ] = 0x8e14177290ecff25415e54d5ed6ede02ef08f64d9bde9687b9fa7b9642d56e9e;
        proofs[1][
        ...
        proofs[2][
            0
        ] = 0x9d6ee2cacdb95d4b802326614bef1f853d37cf48afec52e1b74d01df2eea45d0;
        proofs[2][
            1
        ] = 0xc025cf4f8e2cd25af6209ef3f0de66754f388e98873778417046b3f461742a91;
        proofs[2][
            2
        ] = 0xc82eab709c3962ded202a48d010dfaa80ffb65dca8d1ec1f802823c4604c4ffa;
        proofs[2][
            3
        ] = 0x8c398a265ad9d4df691bf984f2a67c4b087274d2332cb0f41a3ccc4fee2465e9;
        proofs[2][
            4
        ] = 0xc93c4bd617a7d12786e97aa9b3433c6370426dfda54b4c6b9ca64cd796b367b3;
    }

    function getAllocationTypeName(
        VestManager.AllocationType allocationType
    ) internal pure returns (string memory) {
        if (allocationType == VestManager.AllocationType.TREASURY)
            return "TREASURY";
        if (allocationType == VestManager.AllocationType.PERMA_STAKE)
            return "PERMA_STAKE";
        if (allocationType == VestManager.AllocationType.LICENSING)
            return "LICENSING";
        if (allocationType == VestManager.AllocationType.REDEMPTIONS)
            return "REDEMPTIONS";
        if (allocationType == VestManager.AllocationType.AIRDROP_TEAM)
            return "AIRDROP_TEAM";
        if (allocationType == VestManager.AllocationType.AIRDROP_VICTIMS)
            return "AIRDROP_VICTIMS";
        if (allocationType == VestManager.AllocationType.AIRDROP_LOCK_PENALTY)
            return "AIRDROP_LOCK_PENALTY";
        return "UNKNOWN";
    }
    ...
    function test_ClaimWithCallback() public {
        ...
        uint256 claimed = vestManager.claimWithCallback(
            address(this),
            address(autoStakeCallback)
        );
        assertGt(claimed, 0);
    ...
    function test_GetAggregateVestData() public {
        ...
        skip(10 * 365 days);
        (uint256 total, uint256 claimable, uint256 claimed) = vestManager
            .getAggregateVestData(address(this));
