...
contract PairTest is PairTestBase {
    ...
    function test_Borrow() public {
        ...
        addCollateral(
            pair,
            convertToShares(address(collateral), collateralAmount)
        );
    ...
    function test_RedemptionPartial() public {
        ...
        (address reward0, bool isclaimable, uint256 rewardsRemaining) = pair
            .rewards(0);
        ...
        addCollateral(
            otherpair,
            convertToShares(address(otherCollateral), collateralAmount)
        );
        ...
        uint256 totalFee = redemptionHandler.getRedemptionFeePct(
            address(pair),
            redeemAmount
        );
        uint256 nextFee = redemptionHandler.getRedemptionFeePct(
            address(pair),
            1
        );
        ...
        uint256 crvusdprice = underlyingoracle.getPrices(
        ...
        uint256 frxusdprice = underlyingoracle.getPrices(
        ...
        console.log(
            "underlying oracle: ",
            redemptionHandler.underlyingOracle()
        );
        ...
        (
            uint256 previewUnderlying,
            uint256 previewCollateral,
            uint256 previewFee
        ) = redemptionHandler.previewRedeem(address(pair), redeemAmount);
        bool useUnwrap = true;
        ...
        assertEq(
            stablecoinBalBefore - stablecoin.balanceOf(address(this)),
            redeemAmount
        );
        uint256 feesPaid;
        if (totalFee > 0) {
        ...
        for (uint256 i = 0; i < 2; i++) {
            totalFee = redemptionHandler.getRedemptionFeePct(
                address(pair),
                redeemAmount
            );
        ...
        for (uint256 i = 0; i < 5; i++) {
            ...
            console.log("fee used: ", totalFee);
            nextFee = redemptionHandler.getRedemptionFeePct(
                address(otherpair),
                1
            );
            console.log("nextFee: ", nextFee);
            ...
            (, uint256 otherpairUsage) = redemptionHandler.ratingData(
                address(otherpair)
            );
    ...
    function test_RedemptionMax() public {
        ...
        uint256 borrowAmount = 100_000e18;
        uint256 redeemAmount = redemptionHandler.getMaxRedeemableDebt(
            address(pair)
        );
        addCollateral(
            pair,
