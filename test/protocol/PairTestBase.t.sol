...
contract PairTestBase is Setup, ResupplyPairConstants {
    ...
    function printUserInfo(ResupplyPair _pair, address _user) public {
        (uint256 borrowshares, uint256 usercollateral) = _pair.getUserSnapshot(
            _user
        );
    ...
    function printPairFees(ResupplyPair _pair) internal view {
        ...
        console.log(
            "last feedpoist claim epoch: ",
            feeDeposit.lastDistributedEpoch()
        );
    ...
    function ratePerSec(
        ResupplyPair _pair
    ) internal view returns (uint64 _ratePerSec) {
    ...
    function getCurrentLTV(
        ResupplyPair _pair,
        address _user
    ) internal returns (uint256 _ltv) {
        uint256 _borrowerAmount = _pair.toBorrowAmount(
            _pair.userBorrowShares(_user),
            true,
            true
        );
        ...
        _ltv =
            (((_borrowerAmount * _exchangeRate) / EXCHANGE_PRECISION) *
                LTV_PRECISION) /
            _collateralAmount;
    }
    ...
    function getCollateralAmount(
        ...
        _collateralAmount =
            (_borrowAmount * _exchangeRate * LTV_PRECISION) /
    ...
    function calculateMinUnderlyingNeededForBorrow(
        uint256 borrowAmount
    ) public view returns (uint256) {
        return
            (borrowAmount * ResupplyPairConstants.LTV_PRECISION) /
            pair.maxLTV();
    }
