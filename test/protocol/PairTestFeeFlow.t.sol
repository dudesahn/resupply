...
contract PairTestFeeFlow is PairTestBase {
    ...
    */
    function test_feeFlow() public {
        ...
        (uint256 userborrowshares, uint256 usercollateral) = pair
            .getUserSnapshot(address(this));
        (
            uint256 claimblefees,
        ...
        RewardDistributorMultiEpoch.EarnedData[] memory earnedData = pair
            .earned(address(this));
        console.log(
            "govToken rewards before claim: ",
            stakingToken.balanceOf(address(this))
        );
        pair.getReward(address(this));
        console.log(
            "govToken rewards after claim: ",
            stakingToken.balanceOf(address(this))
        );
    ...
    function test_insurancePoolFees() public {
        ...
        console.log(
            "assets in IP: ",
            insurancePool.convertToAssets(insurancePool.balanceOf(_THIS))
        );
        ...
        console.log(
            "govToken on insurance: ",
            stakingToken.balanceOf(address(insurancePool))
        );
        RewardDistributorMultiEpoch.EarnedData[]
            memory earnedData = insurancePool.earned(address(this));
        uint256 rlength = earnedData.length;
        for (uint256 i = 0; i < rlength; i++) {
            console.log(
                "insurance rewards this-> earned token: ",
                earnedData[i].token,
                ", amount: ",
                earnedData[i].amount
            );
        ...
        for (uint256 i = 0; i < rlength; i++) {
            console.log(
                "insurance rewards burnt-> earned token: ",
                earnedData[i].token,
                ", amount: ",
                earnedData[i].amount
            );
        ...
        console.log(
            "withdraw queue: ",
            insurancePool.withdrawQueue(address(this))
        );
    ...
    function printDistributionInfo() internal {
        console.log(
            "stables on feeDeposit: ",
            stablecoin.balanceOf(address(feeDeposit))
        );
        console.log(
            "stables on gov staker: ",
            stablecoin.balanceOf(address(staker))
        );
        console.log(
            "stables on treasury: ",
            stablecoin.balanceOf(address(treasury))
        );
        console.log(
            "stables on insurance: ",
            stablecoin.balanceOf(address(insurancePool))
        );
        console.log(
            "stables on insurance distribution: ",
            stablecoin.balanceOf(address(ipStableStream))
        );
        console.log(
            "govToken on insurance: ",
            stakingToken.balanceOf(address(insurancePool))
        ...
        console.log(
            "govToken on insurance distribution: ",
            stakingToken.balanceOf(address(ipEmissionStream))
        );
        console.log(
            "govToken on pair emissions: ",
            stakingToken.balanceOf(address(pairEmissionStream))
        );
        console.log(
            "govToken on pair: ",
            stakingToken.balanceOf(address(pair))
        );

        (, , , uint256 rate, , ) = staker.rewardData(address(stablecoin));
        console.log("stables reward rate on gov staker: ", rate);
        console.log(
            "stables reward rate on insurancepool: ",
            ipStableStream.rewardRate()
        );
        console.log(
            "emission reward rate on insurancepool: ",
            ipEmissionStream.rewardRate()
        );
        console.log(
            "emission reward rate for all pairs: ",
            pairEmissionStream.rewardRate()
        );
        console.log(
            "weight of current pair: ",
            pairEmissionStream.balanceOf(address(pair))
        );
        console.log("total pair weight: ", pairEmissionStream.totalSupply());
        console.log(
            "emissions earned by pair: ",
            pairEmissionStream.earned(address(pair))
        );
        RewardDistributorMultiEpoch.EarnedData[] memory earnedData = pair
            .earned(address(this));
        uint256 rlength = earnedData.length;
